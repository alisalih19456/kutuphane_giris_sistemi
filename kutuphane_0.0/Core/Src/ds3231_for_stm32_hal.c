...
{
    // Istersen I2C handle'i güncelle (gerekli degilse kaldirabilirsin)
    // _ds3231_ui2c = hi2c;

    time->hours   = DS3231_GetHour();
    time->minutes = DS3231_GetMinute();
    time->seconds = DS3231_GetSecond();
    return 0; // hata kontrolü gerekirse eklenir
}
